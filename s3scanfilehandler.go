...
func validateInputData(w http.ResponseWriter, data *ScanS3Object) error {
...
func S3ScanFileHandler(w http.ResponseWriter, r *http.Request) {
	data := new(ScanS3Object)
	...
	byteData, err := readFile(data.BucketName, data.Key)
	if err != nil {
		elog.Println(err)
		errorResponse(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
	}
	...
	if err != nil {
		...
		if response.data.Status == "INFECTED" {
			...
			err = deleteFile(data.BucketName, data.Key)
			if err != nil {
				elog.Println(err)
				errorResponse(w, err.Error(), http.StatusInternalServerError)
				return
